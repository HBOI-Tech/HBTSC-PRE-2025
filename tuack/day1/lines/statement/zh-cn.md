...
输入的第一行包含一个由小写英文字母构成的字符串 $S$，保证 $1 \le |S| \le {{ args['n'].hn() }}$。

输入的第二行包含一个正整数 $q$，保证 $1 \le q \le {{ args['q'].hn() }}$。

接下来 $q$ 行，每行输入五个由空格分开的正整数 $l_1, r_1, l_2, r_2, k$，保证 $1 \le l_1 + k - 1 \le r_1 \le |S|$，$1 \le l_2 + k - 1 \le r_2 \le |S|$，$1 \le k \le |S|$，描述一组询问。
